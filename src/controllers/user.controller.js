 ...
 try {
    ...
    if(!user){
     ...
     const {accessToken,newrefreshToken} = await generateAccessAndRefreshTokens(user._id) 
      return res.status(200).cookie("accessToken",accessToken,options).cookie("refreshToken",newrefreshToken,options).json(new ApiResponce(200,{accessToken,newrefreshToken},"Access Token refresh successfully"))
 } catch (error) {
...
const updateUserAvatar = asyncHandler(async(req,res)=>{
  ...
  if(!avatarLocalPath){
   ...
   const previousUrl = await User.findById(req.user?._id);
   const previousAvatarUrl = previousUrl.avatar;
 ...
 const user = await User.findByIdAndUpdate(req.user?._id,
    ...
    return res.status(200).json(new ApiResponce(200,user,"Avatar file is updated successfully"))
})
...
const updateUserCoverImage = asyncHandler(async(req,res)=>{
  ...
  const user = await User.findByIdAndUpdate(req.user?._id,
      ...
      {new:true}).select("-password")
      return res.status(200).json(new ApiResponce(200,user,"cover image  file is updated successfully"))
  })
  ...
  const getWatchHistory = asyncHandler(async(req,res)=>{
    ...
    return res.status(200).json(new ApiResponce(200,user[0].watchHistory),"watch history fetched succesfully ")
})
